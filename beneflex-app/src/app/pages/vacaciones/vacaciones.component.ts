...
export class VacacionesComponent implements OnInit {
  ...
  cargarBeneficios() {
    ...
    // ✅ Pro: reintenta hasta que el get-me haya cargado el usuario
    if (!userId) {
      console.warn('Usuario no cargado aún (get-me no ha terminado). Reintentando...');
    ...
    this.beneficiosService.getBeneficios().subscribe({
      next: (data: any) => {
        ...
        // ✅ Mapeo pro: API (snake_case) -> UI model (camel / fields esperados)
